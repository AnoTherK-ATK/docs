...
|`long long`|$-2^{63} ... 2^{63} - 1$|8 bytes|$2^{63} \approx 9 \times 10^{18}$|
...
<Tabs>
    ...
    <TabItem label="Python" icon='seti:python'>
        ...
        <biến> = input()
        ```
        Khi nhập xong thì nó luôn là dạng chuỗi ký tự nên bân cần ép kiểu.
    </TabItem>
</Tabs>
...
Khi mà in số thực ở C++ thì bạn nên thêm các này để nó in ok nhất:

```c++ showLineNumbers=false
cout << fixed << setprecision(<số chữ số sau dấu thập phân>) << x;
```
...
Bằng 2.5 hay là 2.49999999 á? Sai. Nó là 2.
...
Còn với Python thì sẽ là:

```python
x = int(2.5)
```

là ngược lại, ghi tên kiểu trước rồi đóng cái đống phép tính vào ngoặc.
...
- [Chăn bò](https://oj.vnoi.info/problem/olp_kc20_cows). Bài này vận dụng tí nhẹ.
- [Tính toán sát thương](https://lhpoj.io.vn/problem/derank13_dmgcalc). Bạn đọc truyện cho vui.
- Bạn tự luyện tập thêm.
